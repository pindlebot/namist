...
function namer (words = [], options = {}) {
  ...
  let limit = (options.limit / 2) || 50
  let modifiers = options.modifiers || dataset.prefixes.slice(0, limit).map(v => v[0])
  let heads = options.heads || dataset.suffixes.slice(0, limit + 1).map(v => v[0])
